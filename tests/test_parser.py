import unittest
import json
from main import transform_discovery_engine_response, convert_gs_uri_to_proxy_url, app


class TestLayoutParserTransformation(unittest.TestCase):
    """Discovery Engine Layout Parser 응답 변환 로직 단위 테스트"""

    def test_layout_parser_extractive_segments_and_proxy_url(self):
        """1. Layout Parser 본문 세그먼트 추출 및 Cloud Run 이미지 프록시 URL 변환 테스트"""
        mock_raw_response = {
            "results": [
                {
                    "id": "doc-001",
        ...
        result = transform_discovery_engine_response(mock_raw_response, base_url="https://layout-parser-search-api.a.run.app")
        ...
        # 짧고 깔끔한 이미지 프록시 URL 검증
        self.assertEqual(snippet["uri"], "https://layout-parser-search-api.a.run.app/image/cx-manual-bucket/images/filter_cleaning_guide.png")
    ...
    def test_annotation_content_fallback(self):
        ...
        result = transform_discovery_engine_response(mock_raw_response, base_url="https://api.run.app")
        self.assertEqual(len(result["snippets"]), 1)
        self.assertIn("전원 스위치 ON/OFF", result["snippets"][0]["text"])
        self.assertIn("적색 LED", result["snippets"][0]["text"])
        self.assertEqual(result["snippets"][0]["uri"], "https://api.run.app/image/my-bucket/diagrams/wiring.png")

    def test_gs_uri_conversion(self):
        """4. gs:// 경로 변환 검증"""
        self.assertEqual(
            convert_gs_uri_to_proxy_url("gs://my-test-bucket/sub/image.jpg", base_url="https://my-service.run.app"),
            "https://my-service.run.app/image/my-test-bucket/sub/image.jpg"
        )
