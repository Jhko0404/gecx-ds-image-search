    ...
    <step name="Search_And_Answer">
        ...
        <action>
            ...
            3. 검색 결과에 유효한 정보가 매칭되는 경우:
               ...
               - 답변 본문 끝에 반드시 원본 PDF 문서로 연결되는 클릭 가능한 출처 링크([출처: {source} (p.{page})]({pdf_uri}))를 명시하십시오.
               - 시각적 설명에 해당하는 이미지 주소(uri)가 있다면 마크다운 이미지 형식(![매뉴얼 다이어그램]({uri}))으로 출처 링크 바로 아래에 반드시 출력하십시오.
...
#### 💡 핵심 설계 포인트:
- **클릭 가능한 원본 PDF 출처 (Clickable PDF Citation)**: API가 제공하는 `pdf_uri`(V4 서명 링크)를 통해 `[출처: 문서명 (p.페이지)](pdf_uri)` 형식으로 고객이 클릭 시 원본 PDF 문서를 즉시 열람/다운로드할 수 있습니다.
- **다이어그램 이미지 즉시 렌더링**: API가 반환한 V4 Signed URL 이미지 링크를 `![다이어그램](uri)` 마크다운으로 출력하여 웹 채팅창에서 시각 자료가 즉시 표시됩니다.
